	...
	<body>
		...
		<script type="text/javascript">
			...
			function cont_representation(){
				console.log(cont_array.responseJSON);
				var cont_average = [];
				var types = [];
				var array = [];
				var matched = [];
				...
				var a =[];
				var number = [];
				const regex = /\/(\w+)((-|\.)*(\w+))*/;//vgazei to backslash kai . kai - an uparhei
				var b = [];
				...
				for(var i=0; i<cont_array.responseJSON.length;i++){
					number.push(cont_array.responseJSON[i][0]);
				}
				
			
				for(var i=0;i<cont_array.responseJSON.length;i++){	
					if(cont_array.responseJSON[i][1] !== "null"){//Ean den einai null to content type 
						a = cont_array.responseJSON[i][1].match(regex);
						matched[i] = a[0];			
						b.push(Number(cont_array.responseJSON[i][0]));
					}
				}

				
				matched = [... new Set(matched)];//Orizei ta monadika matched
				
				
				for(var j=0;j<types.length;j++){
					if(types[j] !== "null"){
						matched_1[j] = types[j].match(regex);
					}
				}
				
				
				matched = matched.filter(Boolean);//Afairei ta undefined 
				matched_1 = matched_1.filter(Boolean);
				
				console.log(matched_1);
				console.log(matched);
